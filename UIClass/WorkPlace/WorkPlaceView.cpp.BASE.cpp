...
void WorkPlaceView::onAnalysisFinished(QVector<GlobalDataStruct::AnalysisReportInfomation> result)
{
    ...
    if(isALLERGY == true)
    {
        if(BasicSupremeIAnalysis::calcClassData(result, d->mCodeFuncDataBase.data()) == false)
        {
...
void WorkPlaceView::onQRReadCallBack(const QByteArray& recvData)
{
    ...
    case MD_MODE::MD_MODE_CONTROL_WRITE:
    {
        ...
        {
            auto calcMoveDelay = abs(d->mRescaningStripIdx - d->mConfigInstance->getCurrentStripPoint()) + 1;
            int cameraMoveDelay = 2000 * calcMoveDelay;
